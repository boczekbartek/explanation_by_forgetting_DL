...
def get_all_subclasses(ontology: str, workdir: str, out: str):
    ...
    os.system(
        f"sort -u {workdir}/subClasses.nt | grep -v ObjectIntersectionOf | grep -v ObjectSomeV | grep -v example > {out}"
    )
...
def explain_by_forgetting(
    ...
    while len(counter) > 2:
        ...
        if expl_len == 2:
            print(
                f"Minimal explanatiore reached. Ending...", flush=True, file=sys.stderr
            )
...
if __name__ == "__main__":
    ...
    ontology = f"fish_ontology/ontology/fish.owl"
    m = "1"
    args = p.parse_args()
    nforget = args.nforget
    method = args.method
    workdir = f"fish_ontology/workdir_{nforget}_{method}"
    os.makedirs(workdir, exist_ok=True)
    ...
    my_entailment = "<http://bioportal.bioontology.org/ontologies/FISHO#FISHO_0000074> rdfs:subClassOf <http://bioportal.bioontology.org/ontologies/FISHO#FISHO_0000152> ."
